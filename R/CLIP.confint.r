...
CLIP.confint <- function(obj=NULL, variable=NULL, data, firth=TRUE, weightvar=NULL,
  ...
  if (!is.null(obj)) {
    if (is.mira(obj)) {
      ...
      fits<-obj
      if(missing(data)) if(is.null(fits[[1]]$model)) stop("Please provide data either as list of imputed data sets\n")
  ...
  res<-list(
    ...
    imputations=res.tmp$imputations, ci.level=ci.level, bound.lo=bound.LOW,
    bound.up=bound.UPP, iter=iter, call=match.call())
